/* ============================================================
   ...
   Self-contained. Waits for Supabase SDK before running.
   ============================================================ */
...
(function () {
  ...
  /* ---------- BOOT: wait for both DOM + Supabase SDK ---------- */
  function boot() {
    // DOM ready?
  ...
  function tryStart() {
    ...
    if (!window.supabase || typeof window.supabase.createClient !== 'function') {
      console.warn('[reviews] Supabase SDK not ready yet — retrying in 150ms');
      return setTimeout(tryStart, 150);
    }
    ...
    if (!listEl || !summaryEl) {
      console.warn('[reviews] Review section not found in DOM — aborting');
      return;
    }

    startReviews(window.supabase);
  }

  function startReviews(supabaseLib) {
    /* ---------- CLIENT ---------- */
    ...
    function renderSummary(items) {
      ...
      let stars = '';
      for (let i = 1; i <= 5; i++) stars += starSVG();
